...
def list_institutions(country: str = "es", db: Session = Depends(get_db)):
    try:
...
def create_requisition(body: RequisitionCreate, db: Session = Depends(get_db)):
    redirect_url = "http://localhost:8000/api/setup/callback"
    try:
...
def gocardless_callback(ref: str | None = None, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if accounts:
            ...
            db.commit()
            return {"status": "ok", "account_id": accounts[0]}
